...
export class Sheep extends Component {
    ...
    private direction_x:number = 0; // x移動方向
    private direction_y:number = 0; // y移動方向
...
onMouseLeave() {
    ...
    onKeyDown (event: EventKeyboard){
        ...
        case KeyCode. KEY_A:
            this.direction_x = -1; // 往左
        break;
        case KeyCode. KEY_D: 
            this.direction_x= 1; // 往右
        break;
        case KeyCode. KEY_W:
            this.direction_y = 1 // 往上
        break;
        case KeyCode. KEY_S:
            this.direction_y = -1 // 往下
        break;
        }
    ...
    update(deltaTime: number) {
        ...
        if(this.direction_x === 0 && this.direction_y === 0){ // 沒有方向時，停止以下動作
            return;
        }

        if (this.direction_x !== 0) {
    this.node.setScale(-this.direction_x > 0 ? 1 : -1, 1, 1);
}
        const x = this.node.position.x + deltaTime * this.speed * this.direction_x; // 更新 x 位置
        const y = this.node.position.y + deltaTime * this.speed * this.direction_y; // 更新 y 位置
